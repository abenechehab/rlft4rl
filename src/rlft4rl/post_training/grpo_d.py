...
def grpo_function(
    ...
    # Compute observation ranges from the dataset
    all_observations = np.array([sample["observation"] for sample in dataset])
    obs_min = np.min(all_observations, axis=0)
    obs_max = np.max(all_observations, axis=0)
    obs_ranges = [(float(obs_min[i]), float(obs_max[i])) for i in range(len(obs_min))]
    num_obs_dim = len(obs_min)

    # Compute action ranges from the dataset
