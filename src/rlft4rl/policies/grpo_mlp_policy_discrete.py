...
class MLPPolicy(PreTrainedModel):
    ...
    def generate(self, inputs=None, num_return_sequences=1, **kwargs):
        ...
        logits = self.forward(kwargs["input_ids"].float())
...
class PolicyTrainer:
    ...
    def compute_supervised_loss(
        ...
        if self.args.log_prob:
            ...
            log_probs = dist.log_prob(actions_long.squeeze(-1))
            loss = -log_probs.mean()
        else:
            actions_long = actions.long() if actions.dtype != torch.long else actions
            loss = self.criterion(logits, actions_long.squeeze(-1))
        return loss
    ...
    def validate(self, dataloader: DataLoader) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for observations, actions in dataloader:
                ...
                # Compute binary loss: 0 if actions match predicted actions, 1 otherwise
                binary_loss = torch.where(
                    predicted_actions == actions.squeeze(-1), 0.0, 1.0
                )
                total_loss += binary_loss.mean().item()
    ...
    def train(
        ...
        if val_dataloader is not None:
            ...
            if self.ts_writer is not None and val_metrics:
                self.ts_writer.add_scalar("val/loss", val_metrics["val_loss"], -1)
